...
export const createNewWorkspace = async (data: CreateWorkspaceDataType) => {
  try {
    ...
    // redirect(`/workspace/${res[0].workspaceId}`);
    return { data: res };
  } catch (error) {
    console.log(error);
    return {
...
export const updateWorkspace = async (
  ...
  const isUserAMember = await prisma.workspaceMember.findUnique({
    where: {
      userId_workspaceId: {
        userId: user.id,
...
export const removeUserFromWorkspace = async (
  ...
  const isLastOwner = await prisma.workspaceMember.findFirst({
    where: {
      workspaceId: workspaceId,
  ...
  await prisma.workspaceMember.delete({ where: { id: workspaceMemberId } });
...
export const updateProjectAccess = async (
  ...
  const oldProjectAccess = await prisma.projectAccess.findMany({
    where: {
